...
import {
  ...
  SearchOptions,
  SearchResult,
} from '../interfaces/ai-algorithm.interface';
import {
  generateAllMoves,
  isCheckmate,
  isInCheck,
...
export class AdvancedAlgorithm implements IAIAlgorithm {
  ...
  private initOpeningBook(): void {
    ...
    this.openingBook.set('black_response_central_cannon', [
      // Pháo đầu (Trung Pháo phản kích)
      {
        from: { row: 2, col: 7 },
        to: { row: 2, col: 4 },
        piece: { type: PieceType.CANNON, color: PieceColor.BLACK } as Piece,
      },
      // Phi Tượng
      {
        from: { row: 0, col: 2 },
        to: { row: 2, col: 4 },
        piece: { type: PieceType.ELEPHANT, color: PieceColor.BLACK } as Piece,
      },
      // Xuất Mã trái
      {
        from: { row: 0, col: 1 },
        to: { row: 2, col: 2 },
        piece: { type: PieceType.HORSE, color: PieceColor.BLACK } as Piece,
      },
      // Xuất Mã phải
      {
        from: { row: 0, col: 7 },
      ...
      // Xe chạy dọc
      {
        from: { row: 0, col: 0 },
        to: { row: 1, col: 0 },
        piece: { type: PieceType.ROOK, color: PieceColor.BLACK } as Piece,
      },
      {
        from: { row: 0, col: 8 },
        to: { row: 1, col: 8 },
    ...
    this.openingBook.set('red_central_cannon', [
      // Pháo 2 tiến 5 (Trung Pháo)
      {
        from: { row: 7, col: 1 },
        to: { row: 7, col: 4 },
        piece: { type: PieceType.CANNON, color: PieceColor.RED } as Piece,
      },
      // Mã 2 tiến 3
      {
        from: { row: 9, col: 1 },
        to: { row: 7, col: 2 },
        piece: { type: PieceType.HORSE, color: PieceColor.RED } as Piece,
      },
      // Mã 8 tiến 7
      {
        from: { row: 9, col: 7 },
      ...
      // Xe 1 tiến 1
      {
        from: { row: 9, col: 0 },
        to: { row: 8, col: 0 },
        piece: { type: PieceType.ROOK, color: PieceColor.RED } as Piece,
      },
      // Xe 9 tiến 1
      {
        from: { row: 9, col: 8 },
    ...
    this.openingBook.set('red_pawn_opening', [
      // Binh 3 tiến 1 hoặc Binh 7 tiến 1
      {
        from: { row: 6, col: 2 },
        to: { row: 5, col: 2 },
        piece: { type: PieceType.PAWN, color: PieceColor.RED } as Piece,
      },
      {
        from: { row: 6, col: 6 },
        to: { row: 5, col: 6 },
    ...
    this.openingBook.set('red_elephant_opening', [
      {
        from: { row: 9, col: 2 },
        to: { row: 7, col: 4 },
        piece: { type: PieceType.ELEPHANT, color: PieceColor.RED } as Piece,
      },
      {
        from: { row: 9, col: 6 },
        to: { row: 7, col: 4 },
  ...
  findBestMove(
    ...
    if (options?.useIterativeDeepening) {
      ...
      for (let depth = 1; depth <= maxDepth && !this.shouldStop; depth++) {
        ...
        console.log(
          `[Advanced] Depth ${depth}: Score ${result.score}, Nodes ${this.nodesSearched}, Time ${Date.now() - this.startTime}ms`
        );
      }
    } else {
      // Single depth search
  ...
  private searchRoot(
    ...
    alpha: number,
    beta: number
  ): { move: Move | null; score: number | null } {
    const moves = generateAllMoves(board, color);
  ...
  private quiescenceSearch(
    ...
    const moves = generateAllMoves(board, color).filter(
      (m) =>
        m.capturedPiece ||
        isInCheck(
  ...
  private isGoodCapture(move: Move): boolean {
    ...
    if (move.piece.type === PieceType.CANNON) {
      if (
        move.capturedPiece.type === PieceType.HORSE ||
    ...
    if (move.piece.type === PieceType.HORSE && move.capturedPiece.type === PieceType.CANNON) {
      return true;
    }
  ...
  private isTrap(move: Move, board: Board): boolean {
    ...
    for (const response of enemyResponses) {
      ...
      if (responseValue > capturedValue + 100) {
        ...
        const canRecapture = myCounterMoves.some(
          (m) => m.to.row === response.to.row && m.to.col === response.to.col && m.capturedPiece
        );
  ...
  private detectFork(
    ...
    targets: { row: number; col: number; value: number }[];
  }[] {
    const forks: {
      attacker: { row: number; col: number };
      targets: { row: number; col: number; value: number }[];
  ...
  private evaluateExchange(move: Move, board: Board): number {
    ...
    const counterCapture = counterMoves.find(
      (m) => m.to.row === move.to.row && m.to.col === move.to.col && m.capturedPiece
    );
    ...
    if (counterCapture) {
      ...
      if (
        lowestDefenderValue > 0 &&
        lowestDefenderValue < PIECE_VALUES[counterCapture.piece.type]
      ) {
  ...
  private evaluatePieceSafety(board: Board, color: PieceColor): number {
    ...
    for (let row = 0; row < BOARD_ROWS; row++) {
      for (let col = 0; col < BOARD_COLS; col++) {
        ...
        const attackers = enemyMoves.filter((m) => m.to.row === row && m.to.col === col);
        ...
        if (attackers.length > 0) {
          ...
          const defenders = myMoves.filter((m) => m.to.row === row && m.to.col === col);
...
export const ADVANCED_METADATA: AlgorithmMetadata = {
  ...
  name: 'Advanced AI',
  description:
    'Thuật toán AI nâng cao với PVS, Transposition Table, Killer Moves, History Heuristic',
  author: 'System',
  version: '2.0.0',
